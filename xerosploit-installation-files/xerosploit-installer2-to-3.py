...
# Now we can use "raw_input()" in Python3
# Line done by Guilherme Vieira Galvão (GuilhermeVGalvao)
raw_input = input
...
""")
	...
	if system0 == "1":
		...
		install = os.system("apt-get install -y nmap hping3 build-essential python-pip ruby-dev git libpcap-dev libgmp3-dev && pip install tabulate terminaltables")

		install1 = os.system("""cd xerosploit/tools/bettercap/ && gem build bettercap.* && sudo gem install xettercap-* && rm xettercap-* && cd ../../ && mkdir -p /opt/xerosploit && cp -R tools/ /opt/xerosploit/ && cp xerosploit.py /opt/xerosploit/xerosploit.py && cp banner.py /opt/xerosploit/banner.py && cp run.sh /usr/bin/xerosploit && chmod +x /usr/bin/xerosploit && tput setaf 34; echo "Xerosploit has been sucessfuly instaled. Execute 'xerosploit' in your terminal." """)	

		# Xerosploit removes python3-pip to use python-pip,
		# so now we install it again
		# Line done by Guilherme Vieira Galvão (GuilhermeVGalvao)
		python3_pip = os.system('\033[0m' + 'apt-get install -y python3-pip')
	elif system0 == "2":
		print("\033[1;34m\n[++] Installing Xerosploit ... \033[1;m")
